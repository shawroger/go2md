...
{{range .Funcs}}{{$name_html := html .Name}}## <a name="{{$name_html}}">func</a> {{$name_html}}
{{node $ .Decl | pre}}
{{comment_md .Doc}}
{{example_html $ .Name}}
{{callgraph_html $ "" .Name}}{{end}}
{{range .Types}}{{$tname := .Name}}{{$tname_html := html .Name}}## <a name="{{$tname_html}}">type</a> {{$tname_html}}
{{node $ .Decl | pre}}
...
{{range .Methods}}{{$name_html := html .Name}}### <a name="{{$tname_html}}.{{$name_html}}">func</a> ({{md .Recv}}) {{$name_html}}
{{node $ .Decl | pre}}
{{comment_md .Doc}}
